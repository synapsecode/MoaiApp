...
contract MoaiContract {
    ...
    event TargettedPushNotificationSent(
    ...
    function startVoting(address _recipient, uint256 _amount)
        external
        onlyMembers
    {
        require(
        ...
        for (uint256 i = 0; i < memberAddresses.length; i++) {
            ...
            //TODO: Check if they have really contributed; else revert
            m.hasVoted = false;
        }
    ...
    function initiateTransfer(address _recipient, uint256 _amount) external {
        require(_amount > 0, "Amount must be greater than zero");
        ...
        if (totalVotes < totalMembers / 2) {
            ...
            sendTargettedNotification(
                _recipient,
                "Funds Not Transferred",
                "You lost the group vote"
            );
        } else {
            ...
            sendTargettedNotification(
                _recipient,
                "Funds Transferred",
                "You should have recieved your amount"
            );
        }
    }
    ...
    function sendTargettedNotification(
        ...
        IPUSHCommInterface(pushProtocolContractAddress).sendNotification(
    ...
    function sendBroadcast(string memory title, string memory message)
        public
        payable
    {
        IPUSHCommInterface(pushProtocolContractAddress).sendNotification(
